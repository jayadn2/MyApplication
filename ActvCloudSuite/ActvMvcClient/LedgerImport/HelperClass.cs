...
namespace ActvMvcClient.LedgerImport
{
    public class HelperClass
    {
        ...
        {
            bool result = false;
            string[] srcDateArr = null;
            string[] dstDateArr = null;

            //Basically we use 2 types of separators, 1. "-" 2. "/"
            if (dateSrc.Contains("/"))
            {
            ...
            if (dateDst.Contains("/"))
            {
                dateDst = dateDst.Replace('/', '-');
            }
            srcDateArr = dateSrc.Split('-');
            dstDateArr = dateDst.Split('-');
            for (int i = 0; i < srcDateArr.Length; i++)
            {
                if (srcDateArr[i] == dstDateArr[i])
                {
                    result = true;
                }
                else
